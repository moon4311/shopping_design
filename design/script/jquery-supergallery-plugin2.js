...
function delays(gap){ /* gap is in millisecs */ 
  var then,now; 
...
(function($){
	...
	Supergallery.prototype.changeTo = function(n,noAnimation){
		...
		//fade 효과시간으로 인한 이미지 손실을 막기위한 지연시간 2015-12-28 leesb999
		///$(".thumb li").bind('mouseover',function(){
		//	delays(5);
		//});
	...
	var core = {
		...
		superThumbGallery:function(targetSelector,_o){
			...
			$(".btn_class").bind('mouseover',function(){
				main.clearTimer();
			});
			
			$("#m_roll_play").click(function(){
				m_roll_move('Y');
				main.setTimer();
			});
			
			$("#m_roll_stop").click(function(){
				m_roll_move('N');
				main.clearTimer();
			});
			
			$(".thumbPages").bind('mouseover',function(){
				main.clearTimer();	
			});
			
			$("#gallery2, .mainHolder, .thumbHolder, .mRBnr_play").bind('mouseleave',function(){
				if($("#m_roll_play").css('display') == 'block'){
					main.clearTimer();
				}
			});
			
			$(".next_Btn").click(function(){
				var thumb_length = $(".thumbBtn").length
				...
				if(thumb_length < 11){
					...
					}else if(thumb_length == 33){
						if(main.current < 12){
							main.changeTo(11);
						}else if(main.current >= 12 && main.current < 23){
							main.changeTo(22);
						}else{
							main.changeTo(0);
						}
					}else if(thumb_length == 44){
						if(main.current < 12){
							main.changeTo(11);
						}else if(main.current >= 11 && main.current < 23){
							main.changeTo(22);
						}else if(main.current >= 23 && main.current < 34){
							main.changeTo(33);
						}else{
							main.changeTo(0);
						}
